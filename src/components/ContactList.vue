...
export default defineComponent({
  setup() {
    ...
    const getSecondsDb = (timestamp: any) => {
      if (timestamp != undefined) return parseInt(timestamp.seconds);
      else return 1;
    };
    // trae el timestamp de firebase y lo convierte a formato fecha de España.
    const getTimeStamp = (timestamp: any) => {
      let date: Date | undefined;
      if (timestamp != undefined) date = new Date(timestamp.seconds * 1000);
      else date = undefined;
      if (date != undefined)
    ...
    onMounted(async () => {
      const querySnapshot: firebase.firestore.QuerySnapshot<firebase.firestore.DocumentData> =
        await cUsers.where("userUid", "!=", store.state.user.uid).get();
      querySnapshot.forEach(
        async (
          data: firebase.firestore.QueryDocumentSnapshot<firebase.firestore.DocumentData>
        ) => {
          const { photo, name, userUid, date, phoneNumber } = data.data();
          ...
          cMessages.forEach((data) => {
            const { date, type, text, receiverId, senderId, isReceiver } =
              data.data();
            aMessages.push({
              date,
              type,
  ...
  computed: {
    orderedUsers: function (): IUser[] {
      return this.aUsers.slice().sort((a: IUser, b: IUser) => {
        return (
          this.getSecondsDb(b.lastMessageDate) -
